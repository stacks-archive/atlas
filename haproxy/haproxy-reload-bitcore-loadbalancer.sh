...
RESOURCE_GROUP="blockstack-utxo"
LOADBALANCE_TAG="blockstackutxo_loadbalance"
...
VM_TMPFILE="/tmp/${RESOURCE_GROUP}-vms.json"
NIC_TMPFILE="/tmp/${RESOURCE_GROUP}-nics.json"
IPADDR_TMPFILE="/tmp/${RESOURCE_GROUP}-ipaddrs.json"
...
print_backends() {
   ...
   while read vm_name; do
      ...
      echo -n "    server $vm_name $ipaddr_private:3001\\\n"
   done
